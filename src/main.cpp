...
const string strMessageMagic = "Xpax Signed Message:\n";
 ...
 ********************************/
 ...
 int64_t GetProofOfWorkReward(int nHeight, int64_t nFees) {
 	...
 	if (nHeight == 1) {
 	  ...
 	  nSubsidy = 10 * COIN;          // 10.0        XPAX for years 1 - 2
 	}
 	else if (nHeight >= 1036801 && nHeight < 2073601)
 	{
 	  nSubsidy = 5 * COIN;           //  5.0        XPAX for years 3 - 4
 	}
 	else if (nHeight >= 2073601 && nHeight < 3110401)
 	{
 	  nSubsidy = 2.5 * COIN;         //  2.50       XPAX for years 5 - 6
 	}
 	else if (nHeight >= 3110401 && nHeight < 4147201)
 	{
 	  nSubsidy = 1.25 * COIN;        //  1.25       XPAX for years 7 - 8
 	}
 	else if (nHeight >= 4147201 && nHeight < 5184001)
 	{
 	  nSubsidy = 0.625 * COIN;       //  0.625      XPAX for years 9 - 10
 	}
 	else if (nHeight >= 5184001 && nHeight < 6220801)
 	{
 	  nSubsidy = 0.3125 * COIN;      //  0.3125     XPAX for years 11 - 12
 	}
 	else if (nHeight >= 6220801 && nHeight < 7257601)
 	{
 	  nSubsidy = 0.15625 * COIN;     //  0.15625    XPAX for years 13 - 14
 	}
 	else if (nHeight >= 7257601 && nHeight < 8294401)
 	{
 	  nSubsidy = 0.078125 * COIN;    //  0.078125   XPAX for years 15 - 16
 	}
 	else if (nHeight >= 8294401 && nHeight < 9331201)
 	{
 	  nSubsidy = 0.0390625 * COIN;   //  0.0390625  XPAX for years 17 - 18
 	}
 	else if (nHeight >= 9331201 && nHeight < 10368001)
 	{
 	  nSubsidy = 0.01953125 * COIN;  //  0.01953125 XPAX for years 19 - 20
 	}
   LogPrint("POW: Block:  ", "%s\n", nHeight);
   LogPrint("POW: Reward: ", "%s XPAX\n", nSubsidy);
 ...
 int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
 {
     ...
     LogPrint("Masternode Reward:      ", "%s XPAX\n", ret);
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    // xpax: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags, bool fValidateSig)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // xpax: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // xpax: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // xpax: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // xpax: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // xpax: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // xpax: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// xpaxcoin: attempt to generate suitable proof-of-stake
