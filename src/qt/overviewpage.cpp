...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeXpaxAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeXpaxAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeXpaxAmount = strAnonymizeXpaxAmount.remove(strAnonymizeXpaxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXpaxAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeXpaxAmount*COIN) nMaxToAnonymize = nAnonymizeXpaxAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeXpaxAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeXpaxAmount));
        strAnonymizeXpaxAmount = strAnonymizeXpaxAmount.remove(strAnonymizeXpaxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXpaxAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
