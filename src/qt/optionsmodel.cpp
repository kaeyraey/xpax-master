...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeXpaxAmount"))
        settings.setValue("nAnonymizeXpaxAmount", 1000);
    nAnonymizeXpaxAmount = settings.value("nAnonymizeXpaxAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeXpaxAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeXpaxAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeXpaxAmount:
            return QVariant(nAnonymizeXpaxAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeXpaxAmount:
            nAnonymizeXpaxAmount = value.toInt();
            settings.setValue("nAnonymizeXpaxAmount", nAnonymizeXpaxAmount);
            emit AnonymizeXpaxAmountChanged(nAnonymizeXpaxAmount);
            break;
        default:
            break;
        }
