...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: xpax.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: xpaxd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizexpaxamount=<n> " + _("Keep N Xpax anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Xpax is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Xpax is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Xpax version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Xpax server starting\n"); 
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // xpax: reserve balance amount
    {
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Xpax") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Xpax to complete") << "\n";
    ...
    nAnonymizeXpaxAmount = GetArg("-anonymizexpaxamount", 0);
    if(nAnonymizeXpaxAmount > 999999) nAnonymizeXpaxAmount = 999999;
    if(nAnonymizeXpaxAmount < 2) nAnonymizeXpaxAmount = 2;
    ...
    LogPrintf("Anonymize Xpax Amount %d\n", nAnonymizeXpaxAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1XPAX+1000 == (.1XPAX+100)*10
       10XPAX+10000 == (1XPAX+1000)*10
    */
