...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Xpax
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Xpax
    // Mac: ~/Library/Application Support/Xpax
    // Unix: ~/.xpax
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Xpax";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "xpax.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "xpaxd.pid"));
